...
///////////////////
/// MULTIANSWERSET /// (All or Nothing - cloze)
///////////////////
 ...
 */
class embedded_clozeset_qtype extends default_questiontype {

    function name() {
        return 'multianswerset';
    }
    ...
    function get_question_options(&$question) {
        ...
        if (!$sequence = get_field('question_multianswerset', 'sequence', 'question', $question->id)) {
            notify(get_string('noquestions','qtype_multianswerset',$question->name));
    ...
    function save_question_options($question) {
        ...
        if (!$oldwrappedids = get_field('question_multianswerset', 'sequence', 'question', $question->id)) {
                ...
                if (is_array($oldwrappedquestions) && $oldwrappedquestion = array_shift($oldwrappedquestions)) {
                    ...
                    if($oldwrappedquestion->qtype != $wrapped->qtype ) {
                        ...
                        case 'numerical':
                            ...
                            break;
                        default:
                                print_error('qtypenotrecognized', 'qtype_multianswerset','',$oldwrappedquestion->qtype);
                                        $wrapped->id = 0 ;
                            }
                    }
                }else {
        ...
        if (!empty($sequence)) {
            ...
            $multianswer->points = $question->defaultgrade;
            if ($oldid = get_field('question_multianswerset', 'id', 'question', $question->id)) {
                $multianswer->id = $oldid;
                if (!update_record("question_multianswerset", $multianswer)) {
                    $result->error = "Could not update cloze question options! " .
                            "(id=$multianswer->id)";
                    return $result;
                }
            } else {
                if (!insert_record("question_multianswerset", $multianswer)) {
    ...
    function save_question($authorizedquestion, $form, $course) {
        $question = qtype_multianswerset_extract_question($form->questiontext);
    ...
    */
    function delete_question($questionid) {
        delete_records("question_multianswerset", "question", $questionid);
        return true;
    }
           ...
           // $correctanswers = $QTYPES[$wrapped->qtype]->get_correct_responses($wrapped, $state);
            ...
            if ($options->feedback) {
                ...
                if (!empty($chosenanswer->feedback)) {
                    ...
                    if  ($options->readonly && $options->correct_responses) {
                        $strfeedbackwrapped = get_string('correctanswerandfeedback', 'qtype_multianswerset');
                    }else {
            ...
            if ($feedback !='' && $popup == ''){
                $strfeedbackwrapped = get_string('correctanswer', 'qtype_multianswerset');
            ...
            ?>
                ...
                default:
                    ...
                    $a->sub = $positionkey;
                    print_error('unknownquestiontypeofsubquestion', 'qtype_multianswerset','',$a);
                    break;
           }
   //        echo "</label>"; // MDL-7497
        }
            ...
            if(!  isset($question->options->questions[$positionkey])){
                ...
                echo '</label><div class="error" >'.get_string('questionnotfound','qtype_multianswerset',$positionkey).'</div>';
            }
       }
    }
    ...
    function grade_responses(&$question, &$state, $cmoptions) {
        ...
        foreach($question->options->questions as $key => $wrapped) {
            ...
            $correct = $correct && ($teststate->raw_grade == $wrapped->defaultgrade);
        }
        } 
...
/// BACKUP FUNCTIONS ////////////////////////////
    ...
    function backup($bf,$preferences,$question,$level=6) {
        ...
        $multianswers = get_records("question_multianswerset","question",$question,"id");
        //If there are multianswers
        if ($multianswers) {
            //Print multianswers header
            $status = fwrite ($bf,start_tag("MULTIANSWERSETS",$level,true));
            //Iterate over each multianswer
            foreach ($multianswers as $multianswer) {
                $status = fwrite ($bf,start_tag("MULTIANSWERSET",$level+1,true));
                ...
                fwrite ($bf,full_tag("POINTS",$level+2,false,$multianswer->points));
                $status = fwrite ($bf,end_tag("MULTIANSWERSET",$level+1,true));
            }
            //Print multianswers footer
            $status = fwrite ($bf,end_tag("MULTIANSWERSETS",$level,true));
...
/// RESTORE FUNCTIONS /////////////////
    ...
    function restore($old_question_id,$new_question_id,$info,$restore) {
        ...
        $multianswers = $info['#']['MULTIANSWERSETS']['0']['#']['MULTIANSWERSET'];
        ...
        for($i = 0; $i < sizeof($multianswers); $i++) {
			...
			$multianswer->points = $mul_info['#']['POINTS']['0']['#'];
            ...
            $newid = insert_record("question_multianswerset", $multianswer);

            //Save ids in backup_ids
            if ($newid) {
                backup_putid($restore->backup_unique_code,"question_multianswerset",
                             $oldid, $newid);
            }
    ...
    function restore_map($old_question_id,$new_question_id,$info,$restore) {
        ...
        for($i = 0; $i < sizeof($multianswers); $i++) {
            ...
            $db_multianswer = get_record ("question_multianswerset","question",$new_question_id,
            ...
            if ($db_multianswer) {
                ...
                backup_putid($restore->backup_unique_code,"question_multianswerset",$oldid,
                             $db_multianswer->id);
            } else {
                $status = false;
            }
    ...
    function restore_recode_answer($state, $restore) {
        ...
        while ($tok) {
            ...
            if (!$sequence = get_field('question_multianswerset', 'sequence', 'question', $state->question)) {
...
//////////////////////////////////////////////////////////////////////////
question_register_questiontype(new embedded_clozeset_qtype());
...
function qtype_multianswerset_extract_question($text) {
    $question = new stdClass;
    $question->qtype = 'multianswerset';
