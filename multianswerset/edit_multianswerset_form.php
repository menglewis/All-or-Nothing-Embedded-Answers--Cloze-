 ...
 */
class question_edit_multianswerset_form extends question_edit_form {
    ...
    function definition_inner(&$mform) {
        ...
        if  (  "" != optional_param('questiontext','', PARAM_RAW)) {
            ...
            $this->questiondisplay = fullclone(qtype_multianswerset_extract_question(optional_param('questiontext','', PARAM_RAW))) ;
        ...
        $mform->addElement('submit', 'analyzequestion', get_string('decodeverifyquestiontext','qtype_multianswerset'));
        ...
        for ($sub =1;$sub <=$countsubquestions ;$sub++) {
            ...
            $mform->addElement('static', 'sub_'.$sub."_".'questiontext', get_string('questiondefinition','qtype_multianswerset'),array('cols'=>60, 'rows'=>3));
            ...
            if (isset ( $this->questiondisplay->options->questions[$sub]->questiontext)) {
                ...
                if ($this->questiondisplay->options->questions[$sub]->qtype =='multichoice'   ) {
                    $mform->addElement('static', 'sub_'.$sub."_".'layout', get_string('layout', 'qtype_multianswerset'),array('cols'=>60, 'rows'=>1)) ;//, $gradeoptions);
                }
                ...
                foreach ($subquestions as $subquestion) {          
                    ...
                    if ($subquestion->qtype == 'multichoice' ) {
                        ...
                        switch ($subquestion->layout) {
                            case '0':
                                $default_values[$prefix.'layout']= get_string('layoutselectinline', 'qtype_multianswerset');
                                break;
                            case '1':
                                $default_values[$prefix.'layout']= get_string('layoutvertical', 'qtype_multianswerset');
                                break;                         
                            case '2':
                                $default_values[$prefix.'layout']= get_string('layouthorizontal', 'qtype_multianswerset');
                                break;
                            default:
                                $default_values[$prefix.'layout']= get_string('layoutundefined', 'qtype_multianswerset');
                        } 
                    }
        ...
        if (isset($this->questiondisplay->options->questions)) {
           ...
           $subquestions = fullclone($this->questiondisplay->options->questions) ;           
            if (count($subquestions)) {
                ...
                $errors['questiontext']=get_string('questionsmissing', 'qtype_multianswerset');  
            }
        }
