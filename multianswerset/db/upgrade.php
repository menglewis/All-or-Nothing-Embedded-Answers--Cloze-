...
function xmldb_qtype_multianswerset_upgrade($oldversion=0) {
...
/// }
    ...
    if ($result && $oldversion < 2008050800) {
        question_multianswerset_fix_subquestion_parents_and_categories();
    }
 ...
 */
function question_multianswerset_fix_subquestion_parents_and_categories() {
    global $CFG;

    $result = true;
    $rs = get_recordset_sql('SELECT q.id, q.category, qma.sequence FROM ' . $CFG->prefix .
            'question q JOIN ' . $CFG->prefix . 'question_multianswerset qma ON q.id = qma.question');
    if ($rs) {
